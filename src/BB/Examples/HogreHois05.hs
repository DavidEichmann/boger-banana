...
network ds is (node1,node2) = do
        ...
        setDelayedVelocityB ds node2 mouseVelB 3
...
setDelayedVelocityB ds node velocityB delay = do
        ...
        let derive time histDec dt = derivedVel where
                derivedVel      | dt == 0       = (0,0,0)
                                | otherwise     = scale (1) (derive' histInc dt (time-delay))
        ...
        let delVelB = stepper (\x -> scale x initVel) $ (derive <$> tB) <@> velHistoryAbsTE where
