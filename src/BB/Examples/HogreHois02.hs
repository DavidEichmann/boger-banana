module BB.Examples.HogreHois02
where
...
network ds is (node1,node2) = do
        ...
        let getMoveFn keys = bsSum where
                bsSum dt = vecSum (map (\f -> f dt) bs)
                bs = mapMaybe (`lookup` keyBMap) keys
                keyBMap = zip [KC_A,    KC_D,   KC_W,   KC_S]
                              [leftFn,  rightFn, upFn,    downFn]
                              
        let pos1B = accumB (0,0,0) (fmap add (apply (stepper nullFn (getMoveFn <$> keyE)) dtE))
